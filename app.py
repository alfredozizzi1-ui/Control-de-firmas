...
def cargar_datos(nombre_tabla):
    ...
    try:
        ...
        if respuesta.status_code == 200:
            ...
            if not records:
                return pd.DataFrame()
            # Guardamos también el ID interno de Airtable (record id) para poder editar o borrar
            data = []
            for r in records:
                fields = r["fields"]
...
with tab3:
    ...
    if not df_eventos.empty:
        # Crear opciones legibles para elegir el evento a editar
        ...
        with st.form("form_editar_evento"):
            ...
            # Formatear fecha actual para el input de texto
            f_actual = fila_sel.get("fecha", "")
            try:
                f_actual_fmt = datetime.strptime(str(f_actual), "%Y-%m-%d").strftime("%d-%m-%Y")
            except Exception:
                f_actual_fmt = str(f_actual)

            edit_fecha = st.text_input("Fecha (DD-MM-YYYY)", value=f_actual_fmt)
            ...
            if st.form_submit_button("Guardar Cambios"):
                try:
                    fecha_conv = datetime.strptime(edit_fecha.strip(), "%d-%m-%Y").strftime("%Y-%m-%d")
                except ValueError:
